...
本專題旨在透過大數據分析釐清海港自由貿易港區（FTZ）出口貿易價值的核心驅動因子。分析目標是導入 CRISP-DM 數據探勘架構，從 iMarine 資料庫提煉微觀數據，並結合五種機器學習演算法與多模型堆疊集成（Stacking Ensemble）技術，建構出下一期出口貿易值的量化預測模型。預期成果為開發出一套數據驅動的動態招商預測儀表板，協助航港局與港務公司優化招商關鍵績效指標（KPI）並落實精準的政策資源配置。
